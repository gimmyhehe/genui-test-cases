...
import { GetProductDetailArgsSchema, TOOL_NAME, mcpToolDefinitions } from './mcp-server';
import { callGetProductDetail } from './mcp-client';
...
const availableTools = {
  [TOOL_NAME]: {
    definition: mcpToolDefinitions[TOOL_NAME].definition,
    execute: async (args: unknown) => {
      const parsed = GetProductDetailArgsSchema.safeParse(args);
      if (!parsed.success) {
        throw new Error(`Invalid args for ${TOOL_NAME}`);
      }
      return callGetProductDetail(parsed.data.keyword);
    },
  },
} as const;
